...
env00 <- read_dta(paste(getwd(), "ZA3440.dta",sep=""))
env10 <- read_dta(paste(getwd(), "ZA5500_v3-0-0.dta",sep=""))

# CHES
ches10 <- read.csv(paste(getwd(),"2010_CHES_dataset_means.csv", sep = ""), na.strings = "")
ches14 <- read.csv(paste(getwd(),"2014_CHES_dataset_means.csv", sep = ""), na.strings = "")
seat1 <- read.xlsx(paste(getwd(),"Percentage-of-seats-in-parliaments.xlsx", sep = ""),na.strings = "", sheet = 1)
seat2 <- read.xlsx(paste(getwd(),"Percentage-of-seats-in-parliaments.xlsx", sep = ""), na.strings = "", sheet = 2)

# Government spending 
gspend <- read.csv(paste(getwd(),"gov_10a_exp_1_Data.csv", sep = ""))

# CO2 Emission

co2 <- read.table(file = paste(getwd(),"t2020_rd300.tsv", sep=""), sep = '\t', header = TRUE) 

# Environment tax

gtax <- read.csv(paste(getwd(),"env_ac_tax_1_Data.csv", sep = ""))

# GDP per Capita

gdp <- read.csv(paste(getwd(),"nama_10_pc_1_Data.csv", sep = ""))
...
co2 <- co2 %>%
  mutate(
    cntry = car::recode(geo.time, '"EL" = "GR"; "UK" = "GB"'),
...
descr(tab1,show = c("n", "mean", "sd", "range", "skew"),out = "browser")

descr(env_mean,show = c("n", "mean", "sd", "range", "skew"),out = "browser")
...
govpos2$`Goverment position` <- round(govpos2$`Goverment position`,2)
govpos2$`Country name` <- countrycode(govpos2$Country, origin = "iso3n", destination = "country.name")
govpos2 <- govpos2[,c(4,2,3)]
...
weak <- weak <- cfa (mod1, data = env, group = "cntry",
...
# Model 1: whether public opinion predicts government spending
...
summary(m1, fit.measures = TRUE, rsquare = TRUE, standardized = TRUE)
...
"
m1.3 <- sem(model1.3, df, se = "bootstrap")
...
"
m1.4 <- sem(model1.4, df, se = "bootstrap")

summary(m1.4, fit.measures = TRUE, rsquare = TRUE,standardized = TRUE)
...
colnames(tabmod1) <- c("M21", "M22", "M23", "M24")
tabmod1[,1] <- c("-1.210**","0.000", "-0.062","","",32,"0.134","106.782","112.645","-49.391")
tabmod1[,2] <- c("-1.511**","0.000", "-0.034","0.109","",32,"0.163","107.684","115.013","-48.842")
tabmod1[,3] <- c("-1.713**","-0.001", "-0.083","","0.000",32,"0.146","108.333","115.662","-49.167")
tabmod1[,4] <- c("-1.925**","-0.000", "-0.053","0.103","0.000",32,"0.172","109.349","118.143","-48.674")
# Table 3
tab3 <- gt(data = tabmod1,rowname_col = TRUE, rownames_to_stub = TRUE) %>%
  tab_header(
...
# Model 2: whether government positions predicts spending
...
summary(m2, fit.measures = TRUE, rsquare = TRUE)



model2.2 <- "
  # regression
  spend ~ govpos + cntry + year + co2 
"
m2.2 <- sem(model2.2, df)
...
model2.3 <- "
  # regression
  spend ~ govpos + cntry + year + gdp
"
m2.3 <- sem(model2.3, df)
...
model2.4 <- "
  # regression
  spend ~ govpos + cntry + year + co2 + gdp
"
m2.4 <- sem(model2.4, df)

summary(m2.4, fit.measures = TRUE, rsquare = TRUE)
...
colnames(tabmod2) <- c("M1", "M2", "M3", "M4")
tabmod2[,1] <- c("0.104","0.001*", "0.039","","",32,"0.144","59.007","65.112","-25.503")
tabmod2[,2] <- c("0.104","0.001*", "0.039","-0.002","",32,"0.144","61.004","68.636","-25.502")
tabmod2[,3] <- c("0.104","0.001*", "0.039","","0.000",32,"0.144","61.006","68.638","-25.503")
tabmod2[,4] <- c("0.104","0.001*", "0.039","-0.003","0.000",32,"0.144","63.003","72.161","-25.501")

tab4 <- gt(data = tabmod2,rowname_col = TRUE, rownames_to_stub = TRUE) %>%
  tab_header(
...
# Model 3: whether public opinion  predicts spending
...
model3.2 <- "
  # regression
  spend ~ tradeoff + cntry + year + co2
"
m3.2 <- sem(model3.2, df, se = "bootstrap")

summary(m3.2, fit.measures = TRUE, rsquare = TRUE)

model3.3 <- "
  # regression
  spend ~ tradeoff + cntry + year + gdp
"
m3.3 <- sem(model3.3, df, se = "bootstrap")

summary(m3.3, fit.measures = TRUE, rsquare = TRUE)

model3.4 <- "
  # regression
  spend ~ tradeoff + cntry + year + co2 + gdp
"
m3.4 <- sem(model3.4, df, se = "bootstrap")
...
colnames(tabmod3) <- c("M11", "M12", "M13", "M14")
tabmod3[,1] <- c("-0.167","0.001", "0.040","","",32,"0.096","58.052","63.915","-25.026")
tabmod3[,2] <- c("-0.207","0.001*", "0.044","0.015","",32,"0.099","59.962","67.290","-24.981")
tabmod3[,3] <- c("0.271","0.001*", "0.058","","-0.000",32,"0.140","58.462","65.791","-24.231")
tabmod3[,4] <- c("0.229","0.001*", "0.064","0.021","-0.000",32,"0.145","60.272","69.067","-24.136")

## Table 2 

tab2 <- gt(data = tabmod3,rowname_col = TRUE, rownames_to_stub = TRUE) %>%
  tab_header(
...
# Model 4 whether government positions predicts tax revenue


model4 <- "
  # regression
  taxrev ~ govpos + cntry + year 
"
m4 <- sem(model4, df)
...
model4.2 <- "
  # regression
  taxrev ~ govpos + cntry + year + co2
"
m4.2 <- sem(model4.2, df)

summary(m4.2, fit.measures = TRUE, rsquare = TRUE)


model4.3 <- "
  # regression
  taxrev ~ govpos + cntry + year + gdp
"
m4.3 <- sem(model4.3, df)

summary(m4.3, fit.measures = TRUE, rsquare = TRUE)


model4.4 <- "
  # regression
  taxrev ~ govpos + cntry + year + co2 + gdp
"
m4.4 <- sem(model4.4, df)
...
model5 <- "
  # regression
  taxrev ~ tradeoff + cntry + year 
"
m5 <- sem(model5, df, se = "bootstrap")
...
model5.2 <- "
  # regression
  taxrev ~ tradeoff + cntry + year + co2
"
m5.2 <- sem(model5.2, df, se = "bootstrap")

summary(m5.2, fit.measures = TRUE, rsquare = TRUE, standardized = TRUE)
...
"
m5.3 <- sem(model5.3, df, se = "bootstrap")

summary(m5.3, fit.measures = TRUE, rsquare = TRUE, standardized = TRUE)


model5.4 <- "
  # regression
  taxrev ~ tradeoff + cntry + year + co2 + gdp
"
m5.4 <- sem(model5.4, df, se = "bootstrap")
...
colnames(tabmod5) <- c("M31", "M32", "M33", "M34")
tabmod5[,1] <- c("-1.729","0.000", "-0.161","","",32,"0.105","135.451","141.314","-63.726")
tabmod5[,2] <- c("-1.678","0.000", "-0.165","-0.018","",32,"0.106","137.439","144.767","-63.719")
tabmod5[,3] <- c("1.003","0.000", "-0.050","","-0.000**",32,"0.255","131.574","138.902","-60.787")
tabmod5[,4] <- c("0.963","0.000", "-0.045","0.019","-0.000**",32,"0.256","133.557","142.352","-60.779")

## Table 5 
tab5 <- gt(data = tabmod5,rowname_col = TRUE, rownames_to_stub = TRUE) %>%
  tab_header(
...
"
m7.3 <- sem(model7.3, df1, se = "bootstrap")
...
"
m7.4 <- sem(model7.4, df1, se = "bootstrap")
...
"
m8.2 <- sem(model8.2, df1, se = "bootstrap")
...
"
m8.4 <- sem(model8.4, df1, se = "bootstrap")
...
m9 <- sem(modsem3,df1, se = "bootstrap")
